...
// ─── Loading Screen ─────────────────────────────────────
function LoadingScreen({ level, scores, onDone }) {
  ...
  useEffect(() => {
    ...
    trackAssessmentCompleted(totalScore, level, level, {
      ...
      questions_answered: Object.keys(scores).length
    });
...
function LeadCapture({ level, scores, relationshipStatus, onSubmit }) {
  ...
  const handleSubmit = async () => {
    ...
    const leadData = {
      ...
      relationshipStatus,
      scores,
      referralId: generateReferralId(name.trim()), // Generate referral ID for this user
      timestamp: Date.now()
    };
    ...
    // Track lead capture event (existing analytics)
                  ...
                  <FadeIn delay={1250} direction="up">
                    <div className="flex items-center gap-0 bg-gray-800/60 border border-gray-700/50 rounded-xl overflow-hidden focus-within:border-blue-500/50 focus-within:ring-1 focus-within:ring-blue-500/20 transition-all hover:bg-gray-800/80">
                      ...
                      <input
                        ...
                        className="flex-1 bg-transparent py-2.5 pr-4 text-white text-sm focus:outline-none placeholder-gray-500 mp-sensitive"
                      />
                    </div>
                  </FadeIn>
...
function renderShareCard(canvas, level, levelData, relationshipData, percentile, referralLink = null) {
  ...
  ctx.fillText(`What's yours? → ${displayUrl}`, w / 2, h - 32);
}
...
function LevelReveal({ level, scores, insights, relationshipStatus }) {
  ...
  }, [level, relationshipStatus, scores]);
  ...
  const handleLinkedInShare = async () => {
    ...
    try {
      const userLead = window.__aiLevelLead;
      ...
      setTimeout(() => setLinkedinState("idle"), 3000);
    }
  };
  ...
  const handleReserve = async (type) => {
    ...
    // Track Mixpanel CTA clicked
    const totalScore = Object.values(scores)
      .filter(score => typeof score === 'number')
      .reduce((sum, score) => sum + score, 0);
    const ctaText = type === "prove" ? "I want to PROVE it" : "I want to IMPROVE it";
    trackCTAClicked(type, ctaText, "primary", { score: totalScore, level: level });
                ...
                <div className="flex items-start gap-4 bg-gray-900/40 border border-gray-800/60 rounded-2xl p-4 backdrop-blur-sm">
                  ...
                  <div className="flex-1 flex flex-col gap-3">
                    ...
                    <button
                      ...
                      className={`w-full font-bold py-2.5 rounded-xl text-sm flex items-center justify-center gap-2 transition-colors duration-200 mt-2 ${
...
// ─── Main App ───────────────────────────────────────────
function AILevel() {
  // REDIRECT ALL USERS TO NEW NAVIGATION SYSTEM
  useEffect(() => {
    console.log('🔄 Redirecting to new navigation system');
  ...
  // Initialize UTM tracking and referral detection on component mount
  useEffect(() => {
    utmTracker.initialize();

    // Check for referral parameters and track referral visits
    ...
    if (referralId) {
      ...
      // Track in Mixpanel
      import('./mixpanel.js').then(({ trackReferralVisitTracked }) => {
        trackReferralVisitTracked(referralId, {
  ...
  const screenMap = {
    ...
    [SCREENS.LOADING]: <LoadingScreen level={level} scores={scores} onDone={() => setScreen(SCREENS.CAPTURE)} />,
